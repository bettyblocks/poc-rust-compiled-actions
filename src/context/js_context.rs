...
use crate::{
    Context, Value,
    context::{ContextMethods, Scope},
};

type JsMap<'js> = HashMap<Atom<'js>, rquickjs::Value<'js>>;

fn js_to_rs_scope<'js>(
    ctx: &rquickjs::Ctx<'js>,
    mut js_scope: JsMap<'js>,
) -> rquickjs::Result<Scope> {
        ...
        new_map.insert(
            entry
                ...
                .map_err(|_| rquickjs::Error::FromJs {
                    from: "string",
                    to: "u32",
                    message: Some(String::from("Could not parse scope key")),
                })?,
            Value::from_js(ctx, entry.1)?,
        );
    }
...
fn js_map_uint_vars<'js>(
    ctx: &rquickjs::Ctx<'js>,
    mut vars: JsMap<'js>,
    mut uint_mapping: JsMap<'js>,
) -> rquickjs::Result<Scope> {
...
impl JsContext {
    ...
    pub async fn scope<'js>(
        ...
        self.inner
            .borrow_mut()
            .push(js_map_uint_vars(&ctx, input, uint_mapping)?);
        closure
            .call::<(), rquickjs::Value<'js>>(())?
            .into_promise()
            .ok_or(rquickjs::Error::FromJs {
                from: "Scope callback return value",
                to: "Promise",
                message: None,
            })?
            .into_future::<()>()
